...
- **Database Transaction Timeout:** Resolved critical `Lock wait timeout exceeded` error during media-inclusive feedback submission. Optimized database transaction in `Feedback::createWithFacilities` by preparing SQL statements for facility and media inserts once outside the loop, significantly reducing database overhead and transaction time.
...
- **Advanced Blocking Cleanup:** Implemented automatic cleanup of past-dated block records for both resorts and facilities. This ensures the "Current Blocked Dates" display only shows active or future blocks, improving administrative oversight and reducing clutter.

### Files Updated

- `app/Controllers/AdminController.php`
- `app/Models/BlockedResortAvailability.php`
...
- **Admin Booking Management UI:** Added advanced filtering options to the "Unified Booking & Payment Management" page. Administrators can now filter bookings by Payment Status, a specific Customer, Month, and Year, in addition to the existing Resort and Booking Status filters.
- **UI/UX Refinement:** Refined the layout of the new filter section to be more compact and responsive, improving usability and ensuring the interface remains clean on various screen sizes.
...
- **Booking Summary UI/UX:** Iteratively refactored the "Booking Summary" section on the "New Reservation" page for improved readability and a cleaner layout.
  - Replaced the flexbox layout with a centered grid system to position data values next to their corresponding labels, preventing them from being pushed to the far right.
  - Standardized the centered alignment across all static and dynamically generated summary items, including base price, surcharges, and facility details.
  - Implemented logic to display a shortened timeframe label (e.g., "12 Hours") in the pricing line while showing a more detailed description (e.g., "12 Hours - CI: 7:00 AM CO: 5:00 PM") in the main summary section.
...
- **Resort Filter Functionality:** Resolved a series of bugs preventing the resort filter from working correctly across multiple admin and staff pages.
  - **Staff Dashboard:** Corrected a form submission error where the staff dashboard filter was incorrectly pointing to the admin dashboard action.
  - **Admin Dashboard:** Enhanced the "Recent Booking History" table to correctly apply the resort filter, which was previously ignored.
...
- **"Update Booking" Modal Functionality:** Resolved a critical bug in the "Update Booking" modal on the "Unified Booking & Payment Management" page where it failed to display the booking's current status and payment history.
  - **Backend:** The `getBookingDetailsForManagement()` method in `app/Controllers/AdminController.php` was updated to correctly include payment history in its JSON response.
  - **Frontend:** The JavaScript in `app/Views/admin/unified_booking_management.php` was corrected to properly render the fetched payment history and to handle a case-sensitivity issue (`booking.status`) that prevented the booking status from being pre-selected.
